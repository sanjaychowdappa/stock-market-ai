...
/// Combined score from all institutional signals.
#[derive(Debug, Clone, Serialize)]
pub struct InstitutionalSignal {
    pub gex: GammaExposure,
    pub cot: CotData,
    pub volume_profile: VolumeProfile,
    pub cvd: CvdState,
    /// Combined institutional signal: -1 (strong sell) to +1 (strong buy)
...
/// Combine all institutional signals into one composite.
pub fn combine_signals(
    ...
    // CVD (30%) — most real-time, directly measures pressure
    // GEX (25%) — affects how price moves (trending vs mean-reverting)
    // Volume Profile (25%) — S/R levels based on actual volume
    // COT (20%) — weekly, shows big picture positioning
    let composite = 0.30 * cvd.signal
        + 0.25 * gex.signal
        + 0.25 * vp.signal
        + 0.20 * cot.signal;

    // Confidence: higher when signals agree
    let signals = [cvd.signal, gex.signal, vp.signal, cot.signal];
    let all_positive = signals.iter().all(|&s| s > 0.0);
    let all_negative = signals.iter().all(|&s| s < 0.0);
    let agreement = if all_positive || all_negative { 0.9 } else {
        let same_sign = signals.iter().filter(|&&s| s.signum() == composite.signum()).count();
        same_sign as f64 / 4.0
    };

    let summary = format!(
        "CVD:{} GEX:{} VP:{} COT:{} → {}",
        if cvd.signal > 0.2 { "BUY" } else if cvd.signal < -0.2 { "SELL" } else { "FLAT" },
        gex.regime,
        vp.position,
        if cot.signal > 0.2 { "BULL" } else if cot.signal < -0.2 { "BEAR" } else { "NEUT" },
        if composite > 0.3 { "BULLISH" } else if composite < -0.3 { "BEARISH" } else { "NEUTRAL" },
    );

    InstitutionalSignal {
        gex: gex.clone(),
        cot: cot.clone(),
        volume_profile: vp.clone(),
        cvd: cvd.clone(),
        composite_signal: composite.clamp(-1.0, 1.0),
